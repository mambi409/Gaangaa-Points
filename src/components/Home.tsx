...
interface HomeProps {
  stores: StoreType[];
  authUser?: { username: string; name: string; email?: string; role?: 'user' | 'merchant' | 'admin' } | null;
...
export const Home: React.FC<HomeProps> = ({
  ...
  const { t } = useLanguage();
  const isMerchantUser = authUser?.role === 'merchant' || currentRole === 'merchant';
      ...
      <section className="relative overflow-hidden rounded-3xl text-white p-8 sm:p-12 md:p-16 shadow-2xl border border-slate-800 bg-slate-950">
        ...
        <div className="relative z-10 max-w-3xl space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            {isMerchantUser ? (
              <button
                onClick={onGoToDashboard || onOpenMerchantAuth}
                className="px-6 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-xs sm:text-sm rounded-xl transition shadow-lg shadow-indigo-600/30 flex items-center gap-2 cursor-pointer"
              >
            ...
            <button
              onClick={isMerchantUser && onGoToDashboard ? onGoToDashboard : onOpenMerchantAuth}
            ...
            >
              ...
              <span>{isMerchantUser ? 'Merchant Dashboard' : t('home.btn_merchant')}</span>
            </button>
          </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-md space-y-6 flex flex-col justify-between">
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            >
              <Building2 className="w-4 h-4" />
              <span>{isMerchantUser ? 'Open Merchant Dashboard' : t('home.merchant_cta_btn')}</span>
            </button>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {stores.slice(0, 3).map((store) => {
            const status = getStoreOpenStatus(store, new Date(), (t('stores.points_rate').includes('puntos') ? 'es' : 'en'));
            ...
            return (
              ...
              >
                <div className="relative h-44 overflow-hidden bg-slate-100 dark:bg-slate-800">
                  ...
                  {/* Category Pill & Open Status */}
                  <div className="absolute top-3 left-3 flex items-center gap-1.5">
                    <div
                      className={`px-2.5 py-1 rounded-lg text-[11px] font-extrabold flex items-center gap-1 shadow-xs backdrop-blur-md ${
                        status.isOpen
                    ...
                    <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md px-2.5 py-1 rounded-lg text-[11px] font-extrabold text-slate-800 dark:text-slate-200 shadow-xs">
                      {store.category}
                    </div>
                  </div>
                  ...
                  {store.logo && (
                    <img
                      src={store.logo}
                      alt={`${store.name} logo`}
                      className="absolute bottom-3 left-3 w-9 h-9 rounded-xl object-cover bg-white p-0.5 border border-white shadow-md"
                    />
                  )}
                </div>
                ...
                <div className="p-5 space-y-3">
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
                    <div className="flex items-center gap-1 font-semibold">
                      ...
                      <span>{status.hoursDisplay}</span>
                    </div>
                    <span className={`text-[11px] font-bold ${status.isOpen ? 'text-emerald-600' : 'text-rose-600'}`}>
